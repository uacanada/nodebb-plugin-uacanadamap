...
setupApiRoute(  router,"get",  "/uacanadamap/setdefaults", onlyAdmin, async (req, res) => {
  await setDefaultSettings()// TODO remove
  helpers.formatApiResponse(200, res, { message: "setdefaults" }); 
})
...
async function setDefaultSettings(){
  ...
  if (!settingsExist) {
    try {
      ...
      console.log('Error getting settings file',error)
  ...
  try {
    const defaults = await meta.settings.set("uacanadamap", newSettings);
    return defaults;
  } catch (error) {
