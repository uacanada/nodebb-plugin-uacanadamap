...
setupApiRoute(  router,"get",  "/uacanadamap/flushsettings/:confirmation", onlyAdmin, async (req, res) => {
  let recovered = null
  ...
  try {
    ...
    const confirmedResetToDefaults = confirmation === "I confirm the resetting and recreation of settings from defaultSettings.json";
    ...
    if(confirmedResetToDefaults || confirmedTotalDeletion){
      recovered = await setDefaultSettings({forced:true})
    }

    if(confirmedTotalDeletion){
      try {
          placesDeletion = await db.delete("uacanadamap:places");
    ...
    helpers.formatApiResponse(200, res, { sets, updated, recovered, placesDeletion});
  } catch (error) {
...
async function setDefaultSettings({forced}) {
  try {
    ...
    const defaultSettingsPath = path.resolve(  __dirname,  '..',  '..', 'settings', 'defaultSettings.json' );
