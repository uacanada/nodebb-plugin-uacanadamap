...
Plugin.addRoutes = async ({ router, middleware, controllers, helpers }) => {
  routeHelpers.setupApiRoute( router,"get",  "/uacanadamap/reset/:confirmation", async (req, res) => {
    winston.error('get uacanadamap');
    try {
      helpers.formatApiResponse(200, res, { result:'OK', confirm:req.params?.confirmation, res }); // Debug
    } catch (error) {
      winston.error(error);

      helpers.formatApiResponse(200, res, { result:'ERR', confirm:req.params?.confirmation, res }); // Debug
    }
  })
};
...
Plugin.init = async ({ router, middleware, controllers, helpers } ) => {
  ...
  routeHelpers.setupApiRoute(  router,"get",  "/map/pluginsettings/flushsettings/:confirmation",
    middlewares, // TODO
    async (req, res) => {
      try {
