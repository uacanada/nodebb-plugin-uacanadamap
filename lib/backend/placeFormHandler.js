...
async function handleAddPlaceRequest(req, res, helpers) {
    ...
    try {
        ...
        async function processUploadedFile(req, tid) {
            if (!req.files || !req.files.image) return;
        
            try {
                const uploadedFile = await uploadFile(req, 'image', '/public/uploads/');
        ...
        function uploadFile(req, name, path) {
            return new Promise((resolve, reject) => {
                file.saveFileToLocal(req, name, path, (err, uploadedFile) => {
        ...
        async function adaptImage(filename, fileExtension, uploadedFile, tid) {
            const { url, path: imagePath } = await file.saveFileToLocal(filename + fileExtension, 'files', uploadedFile.tempPath);
        ...
        processUploadedFile(req, tid).catch(err => winston.warn(err.message));
