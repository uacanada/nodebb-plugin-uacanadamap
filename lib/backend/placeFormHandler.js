    ...
    try {
        ...
        const { tid, timestamp, slug, user } = topic.topicData ? topic.topicData : topic;
            ...
            const allowedExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.tiff', '.heic', '.bmp'];
                ...
                async function uploadAndProcessImage(req, tid) {
                    const uploadedFile = await file.upload(req, 'image', '/public/uploads/');

                    if (!uploadedFile) {
                        throw new Error("No file was uploaded.");
                    }

                    const fileExtension = getFileExtension(uploadedFile.mimetype);
                    validateFileExtension(fileExtension);

                    const filename = generateFilename(req, tid);
                    await adaptImage(filename, fileExtension, uploadedFile, tid);
                }

                function getFileExtension(mimetype) {
                    return file.typeToExtension(mimetype).toLowerCase();
                }
                ...
                function generateFilename(req, tid) {
                    return `${Math.floor(Date.now() / 1000)}u${req.uid}t${tid}`;
                }

                async function adaptImage(filename, fileExtension, uploadedFile, tid) {
                    try {
                        const { url, path: imagePath } = await uploadImage(`${filename}${fileExtension}`, 'files', uploadedFile);
                        const thumbnailPath = generateThumbnailPath(imagePath, fileExtension);

                        await resizeAndAssociateImage(imagePath, thumbnailPath, tid, filename, fileExtension);
                    } catch (error) {
                        handleImageError(error, filename, fileExtension, tid);
                    }
                }

                function generateThumbnailPath(imagePath, fileExtension) {
                    return imagePath.replace(fileExtension, `_thumb${fileExtension}`);
                }

                async function resizeAndAssociateImage(imagePath, thumbnailPath, tid, filename, fileExtension) {
                    await resizeImage({
                        path: imagePath,
                ...
                function handleImageError(error, filename, fileExtension, tid) {
                    if (error.message.includes("resize")) {
                        notice.thumbErr = `thumbnail error ${error.toString()}`;
                        topics.thumbs.associate({ id: tid, path: `/files/${filename}${fileExtension}` }).catch(err => winston.error(err));
                    } else {
                ...
                if (req.files && req.files.image) {
                    uploadAndProcessImage(req, tid).catch(err => winston.warn(err.message));
                }
