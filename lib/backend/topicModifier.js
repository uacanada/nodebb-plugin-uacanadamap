...
module.exports = (data, settings) => {
  try {
    ...
    if (mapFields?.placeTitle) {
      ...
      const authorAvatar = author.picture ? `<img alt="${author.username}" title="${author.username}" data-uid="${author.uid}" loading="lazy" class="avatar avatar-rounded" component="user/picture" src="${author.picture}" style="--avatar-size: 24px;" onerror="this.remove();" itemprop="image">`: `<div style="background-color:${authorColor}">${authorLetter}</div>`
      ...
      <nav id="placeTopicNav" class="pb-2 m-0">
        ...
        <ul class="fs-6 list-group list-group-flush m-0 p-0">
          ...
          <li class="list-group-item">${authorAvatar} ${author.displayname || author.username}</li>
