  ...
  try {
    ...
    const eventWeekDay = m.eventWeekDay ? `<p class="calendar-recurrence m-0"><i class="fa-solid fa-rotate-right"></i> Every ${m.eventWeekDay} at ${m.eventStartTime}</p>` : "";
      ...
      if(m.gallery && m.gallery[0]){
        try {
          let galleryitems = "";
          for(let imgsrc of m.gallery){ galleryitems += `<div class="swiper-slide"><div class="swiper-zoom-container"><img alt="Place Gallery Image" class="p-0 m-0" src="/assets/uploads${imgsrc}"/></div></div>` }
