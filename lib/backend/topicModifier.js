...
module.exports = (data, settings) => {
  try {
    ...
    if (mapFields?.placeTitle) {
      ...
      const topicMapDiv = `<div id="topicMap" class="shadow text-nowrap w-100 mb-3 rounded" style="height:12rem" data-ua-latlng="${m.latlng}"></div>`;
      ...
      // Define conditional sections first to make the main template more readable.
        ...
        const liAttrs = 'class="nav-item" role="presentation"'
      ...
      const authorAvatar = author.picture ? `<img alt="${author.username}" title="${author.username}" data-uid="${author.uid}" class="p-0 m-0 avatar avatar-rounded" component="user/picture" src="${author.picture}" style="--avatar-size: 24px;" onerror="this.remove();" itemprop="image">`: `<span class="ratio ratio-1x1 rounded-circle" style="width:1rem;color:white;background-color:${authorColor}">${authorLetter}</span>`

     

      

      const modifiedContent = `<div class="placeTopicMetaWrapper p-2 mb-3 rounded shadow">
      
      <nav id="placeTopicNav" class="pb-2 m-0">
          <ul class="nav nav-pills" id="top-pills-tab" role="tablist">
             ${eventHtml.button} 
              <li ${liAttrs}>
        ...
        <div id="metaTab" class="tab-content">
           ${eventHtml.tab}
            <div class="tab-pane fade mb-3${eventHtml.contactsClass?' active show':''}" id="navProfile" role="tabpanel" aria-labelledby="navProfile-tab" tabindex="0">
              <ul class="fs-6 list-group list-group-flush m-0 p-0">
                ${contactsList}
              </ul>
            </div>
        ...
        <ul class="fs-6 list-group list-group-flush m-0 p-0">
          <li class="list-group-item">${(m.categoryName && m.categoryName !== 'undefined') ? m.categoryName : m.placeCategory}</li>
          <li class="list-group-item">${(m.categoryName && m.categoryName !== 'undefined') ? m.categoryName : m.placeCategory}</li>
          <li class="list-group-item">${(m.eventCategory) ? (m.eventCategoryName && m.eventCategoryName !== 'undefined') ? m.eventCategoryName : m.eventCategory : '' }</li>
          <li class="list-group-item" style="color:${authorColor}">${authorAvatar} ${author.displayname || author.username}</li>

          
        </ul>

        <ul class="nav nav-pills" id="bottom-pills-tab" role="tablist">
              <li ${liAttrs}>
                  <button class="active ${btnClasses}" title="Show article" id="navDescription-tab" data-bs-target="#navDescription" ${btnAttrs} aria-controls="navDescription" aria-selected="true"><i class="fa fa-solid fa-circle-info"></i></button>
              </li>
              <li ${liAttrs}>
                  <button class="${btnClasses}" title="Show alternative lang" id="navAltDescription-tab" data-bs-target="#navAltDescription" ${btnAttrs} aria-controls="navAltDescription" aria-selected="false"><i class="fa-solid fa-language me-3"></i> ${settings.altContentTitle}</button>
              </li>
          </ul>
