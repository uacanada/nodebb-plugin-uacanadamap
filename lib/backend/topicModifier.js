    ...
    if (data.topic?.mapFields?.placeTitle) {
      ...
      const accordionPlace = `<div class="accordion text-nowrap m-0 p-0 my-3" id="placeFields">
        ...
        <div class="tab-content" id="nav-tabContent" style="white-space: normal;">
          <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab" tabindex="0">${m.placeDescriptionAlt ? altDescription : ""}</div>
      ...
      //data.topic.content = topicMapDiv + adminDebugConsole + topicEventDiv + placeDiv + data.topic.content;
      data.topic.posts[0].content =  topicMapDiv + adminDebugConsole + topicEventDiv + placeDiv + data.topic.posts[0].content;
    }
  } catch (error) {
    // TODO error logging
  }
