  ...
  try {
    ...
    const eventWeekDay = m.eventWeekDay ? `<p class="calendar-recurrence m-0"><i class="fa-solid fa-rotate-right"></i> Every ${m.eventWeekDay} at ${m.eventStartTime}</p>` : "";
      ...
      if(m.gallery && m.gallery[0]){
        try {
          let galleryitems = "";
          for(let imgsrc of m.gallery){ galleryitems += `<div class="swiper-slide"><div class="swiper-zoom-container"><img style="height: 17rem; width: auto;" alt="Place Gallery Image" src="/assets/uploads${imgsrc}"/></div></div>` }
          gallerySwiper = `<div id="topicPlaceGallery" class="swiper"><div class="swiper-wrapper">${galleryitems}</div></div>`
        } catch (error) {
          console.log(error,m.gallery)
        }
      }
