...
module.exports = (data, settings) => {
  try {
    ...
    const eventCategoryLi = m.eventCategory ? `<span class="badge text-bg-info me-3">${(m.eventCategory) ? (m.eventCategoryName && m.eventCategoryName !== 'undefined') ? m.eventCategoryName : m.eventCategory : '' }</span>`:''
    const placeCategory = `<span class="badge text-bg-info me-3">${(m.categoryName && m.categoryName !== 'undefined') ? m.categoryName : m.placeCategory}</span>`
      ...
      <nav id="placeTopicNav" class="pb-2 m-0">
        ...
        <div id="descTab" class="tab-content">
          <div class="tab-pane fade mb-3 active show" id="navDescription" role="tabpanel" aria-labelledby="navDescription-tab" tabindex="0">
           <div class="white-space-pre-line">${eventCategoryLi}${placeCategory}${data.topic.posts[0].content}</div>
          </div>
