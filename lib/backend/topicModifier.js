...
module.exports = (data, settings) => {
  try {
    ...
    if (mapFields?.placeTitle) {
      ...
      const authorAvatar = author.picture ? `<img alt="${author.username}" title="${author.username}" data-uid="${author.uid}" class="p-0 m-0 avatar avatar-rounded" component="user/picture" src="${author.picture}" style="--avatar-size: 24px;" onerror="this.remove();" itemprop="image">`: `<div style="background-color:${authorColor}">${authorLetter}</div>`
