    ...
    if (mapFields?.placeTitle) {
      ...
      <div id="metaTab" class="tab-content" data-authorcolor="${authorColor}" data-authoravatar="${authorAvatar}" data-authorname="${author.displayname || author.username}" data-authorletter="${authorLetter}">
        ...
        <div class="place-ctx-buttons d-flex justify-content-end p-2">
          ...
          <a href="/topic/${m.tid}"  title="Comment this" type="button" class="btn circle-button go-to-comment ms-2 p-4 text-bg-primary"><i class="fa-regular fa-comment-dots"></i></a>
          </div>
        </div>
        `;
