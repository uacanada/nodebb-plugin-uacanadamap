  ...
  try {
    ...
    if (mapFields?.placeTitle) {
      ...
      const createListElement = (icon, text, color = "") => {
        return `<li class="list-group-item mx-0 px-0">
      ...
      const altDescription = m.placeDescriptionAlt
        ...
        const btnClasses = 'nav-link badge rounded-pill text-bg-primary me-3'
      ...
      const modifiedContent = `<div class="placeTopicMetaWrapper p-2 mb-3">
      
      <nav id="placeTopicNav" class="pb-2 m-0 d-flex justify-content-between">
        ...
        <div id="metaTab" class="tab-content">
           ${eventHtml.tab}
            ...
            <div class="tab-pane fade mb-3" id="navAddress" role="tabpanel" aria-labelledby="navAddress-tab" tabindex="0">
                <ul class="fs-6 list-group list-group-flush m-0 p-0">
                ${ m.city ? `<li class="list-group-item mx-0 px-0">${m.city}, ${m.province}</li>` : "" }
                ${ m.streetAddress ? `<li class="list-group-item mx-0 px-0">${m.streetAddress}</li>`  : "" }
                ${ m.latlng && m.latlng.length === 2
                    ? `<li class="list-group-item mx-0 px-0"> <a title="google map" href="https://maps.google.com/?q=${latlngString}"><i class="fa-brands fa-google"></i> ${latlngString}</a></li>`
                    : ""
                }
              </ul>
            </div>
