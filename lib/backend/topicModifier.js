    ...
    const eventWeekDay = m.eventWeekDay ? `<p class="calendar-recurrence m-0"><i class="fa-solid fa-rotate-right"></i> Every ${m.eventWeekDay} at ${m.eventStartTime}</p>` : "";
      ...
      if(m.gallery && m.gallery[0]){
        try {
          ...
          for(let imgsrc of m.gallery){
            galleryitems += `<div class="swiper-slide"><img src="${imgsrc}"></div>`
          }

          gallerySwiper = `<div class="swiper mySwiper">  <div class="swiper-wrapper">  ${galleryitems} </div> </div>`
      ...
      data.topic.posts[0].content = `${topicMapDiv}${modifiedContent}`;
    }
