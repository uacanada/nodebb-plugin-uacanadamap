...
module.exports = (data, settings) => {
  try {
    ...
    const eventCategoryLi = m.eventCategory ? `<li class="list-group-item"><span class="badge text-bg-info">${(m.eventCategory) ? (m.eventCategoryName && m.eventCategoryName !== 'undefined') ? m.eventCategoryName : m.eventCategory : '' }</span></li>`:''
      

      const modifiedContent = `<div class="placeTopicMetaWrapper p-2 mb-3 rounded shadow">
      
      <nav id="placeTopicNav" class="pb-2 m-0">
          <ul class="ps-2 list-unstyled nav nav-pills" id="top-pills-tab" role="tablist">
             ${eventHtml.button} 
              <li ${liAttrs}>
        ...
        <ul class="fs-6 list-group list-group-flush m-0 p-0">
        ${eventCategoryLi}
          <li class="list-group-item"><span class="badge text-bg-info">${(m.categoryName && m.categoryName !== 'undefined') ? m.categoryName : m.placeCategory}</span></li>
          
          <li class="list-group-item" style="color:${authorColor}">${authorAvatar} ${author.displayname || author.username}</li>
          <li class="list-group-item">
              <div role="tablist">
                      
                    <a href="#" class="active me-2" title="Show article" id="navDescription-tab" data-bs-target="#navDescription" ${btnAttrs} aria-controls="navDescription" aria-selected="true"><i class="fa fa-solid fa-circle-info"></i></a>
                    <a href="#" title="Show alternative lang" id="navAltDescription-tab" data-bs-target="#navAltDescription" ${btnAttrs} aria-controls="navAltDescription" aria-selected="false"><i class="fa-solid fa-language me-3"></i> ${settings.altContentTitle}</a>
